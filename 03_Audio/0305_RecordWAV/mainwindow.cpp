...
void MainWindow::audioDurationChanged(unsigned long long newAudioDuration) {
    QTime time(0, 0, 0, 0);
    QString text = time.addMSecs(newAudioDuration).toString("mm:ss.z").left(7);
    qDebug() << "aaa" << ui->timeLabel;
    qDebug() << "bbb" << ui->pushButton;
...
//                    _audioRecordThread->setStop(true);
        ...
        connect(_audioRecordThread, &AudioRecordThread::audioDurationChangned,
                this, &MainWindow::audioDurationChanged);
...
// 录音注意点四：录音线程本身的内存溢出问题
// 好的，上面其实已经基本实现了开始录音和停止录音的逻辑，但是有一个问题：我们每点一次开始录音就会创建一个音频录制线程对象，
// 而每个音频线程对象的生命周期都是跟当前MainWindow保持一致的，所以只有在关闭窗口的时候这些线程才会一起销毁，那如果频繁
// 地点录音不就会导致音频线程对象越来越多，很有可能导致内存溢出吗？是的！
// 1、你想到的第一个办法可能是让音频线程对象懒加载，不是nullptr再创建，但这种做法对线程来说是不可行的，我们上面也说过了
// 一个线程在执行结束后虽然它的内存还存在，但是它已经不能被再次使用了，所以没办法我们还就必须每次开始录音都创建一个新的音频
// 录制线程
// 2、1不可行的话，我们能想到的第二个办法就是在点击“停止录音”时导致音频录制线程执行结束的时候主动释放一下该音频线程对应的内存，这个
// 办法可行的，我们只需要监听一下音频录制线程执行结束就可以了，详见audiorecordthread.cpp里的构造函数
