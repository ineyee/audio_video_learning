...
void AudioRecordThread::run() {
   ...
   // 这里存到.wav文件里
   // 直接用路径创建一个栈区的文件对象
   // 因为C语言的字符串不能直接相加，所以我们这里用Qt提供的一个字符串，当然我们也可以用C++的字符串
   QString fileName = QDateTime::currentDateTime().toString("yyyy_MM_dd_HH_mm_ss");
   QString suffix = ".wav";
   QString filePath = OUTPUT_FILE_PATH + fileName + suffix;
   QFile file(filePath);
   ...
   // 我们用Mini外置麦克风录音设备录音的参数是这些，这个demo我们先写死这些参数
   WAVHeader header;
   // Mini 0号设备
...
//   while (_stop == false) {
    while (isInterruptionRequested() == false) {
       ...
       if (ret == 0) { // 这一次采集成功
           ...
           // 累加录音时长
           audioDuration += (pkt.duration / 1000.0);
           // 发送信号
           emit audioDurationChangned(audioDuration);
    ...
    // pcm数据的大小
    header.pcmDataSize = file.size() - sizeof(WAVHeader);
    // 如果文件头是严格的44个字节的话，这4个字节数据的下标应该是从[40, 43]，我们直接写死seek(40)也可以
    // 但严谨起见还是让它动态的计算吧，避免FFmpeg自己往文件头里插入东西
    // 我们知道这段数据肯定是占4个字节，而且也肯定是在文件头的最后4个字节，所以直接seek(文件头的大小 - 4)即可
    file.seek(sizeof(WAVHeader) - sizeof(header.pcmDataSize));
    // 跳过来之后把pcm数据的大小写进这4个字节里
    // 要写什么数据进去，就把这个数据的起始位置给到write函数，并且用const char表明是一个字节一个字节往里写，再告诉write函数应该要写多少个字节就ok了
    file.write((const char *)&header.pcmDataSize, sizeof(header.pcmDataSize));

    // 其实就是整个.wav文件的大小 - 4 - 4，只不过为了严谨起见，不写死而已
    header.riffDataSize = file.size() - sizeof(header.riffID) - sizeof(header.riffDataSize);
    file.seek(sizeof(header.riffID));
    file.write((const char *)&header.riffDataSize, sizeof(header.riffDataSize));
