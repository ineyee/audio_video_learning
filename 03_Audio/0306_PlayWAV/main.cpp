...
// 这一个Demo的目标是：在子线程中进行音频播放，播放上面转换到的.wav文件或者录制到的.wav文件
// 既然是播放，这一个Demo就用不到FFmpeg，只有SDL就够了，所以我们先去.pro文件里配置一下SDL库

// 其实也没什么难的，我们知道播放.pcm文件无非就是我们手动设定好PCM数据的各种参数，然后从.pcm文件里读取PCM数据播放就可以了
// 那播放.wav文件也是一样的道理，唯一的区别就是：设置各种参数的时候不用我们写死了，而是可以从.wav文件的文件头里读取出来设置，而SDL已经提供了相关的API

// 我们再添加个记录播放时长功能，注意不要用timer去记录播放时长，因为timer和播放线程是在两个线程里的，两者之间的同步性很成问题，
// 所以用timer记录出来的时间绝对不准，我们应该直接用播放掉的数据算出来到底播放了多长时间才是最准确的
// 当然“用播放掉的数据算出来到底播放了多长时间”是在音频播放线程里才能获取到的，我们如果想把这个数据交给主线程，可以采用回调或者信号槽的方式，这里我们就采用信号槽来实现一下
//
// label注意搞下等宽字体，不然播放时长会看着闪来闪去
